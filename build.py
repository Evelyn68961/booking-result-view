...
function renderBatch() {
  ...
  body.innerHTML = state.batch.map(e => {
    ...
    return `<tr>
      ...
      <td class="date">${escapeHtml(e.start || '')}</td>
      <td class="date">${escapeHtml(e.end || '')}</td>
      <td class="num">${days}</td>
      <td>${escapeHtml(e.daysCat || '')}</td>
...
function showRange() {
  const w = roundWindow();
  if (w) $('rangeMeta').textContent = `本輪預約範圍：${w.from} ～ ${w.to}（含）`;
  else $('rangeMeta').textContent = '本輪預約範圍：尚未設定（請於管理員區設定）';
}
